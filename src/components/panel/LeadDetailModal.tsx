...
export function LeadDetailModal({ lead, onClose }: LeadDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/65 backdrop-blur-sm overflow-y-auto">
      <div
        className="relative bg-white dark:bg-slate-900 rounded-3xl max-w-3xl w-full max-h-[92vh] overflow-y-auto shadow-2xl border border-slate-200 dark:border-slate-800 p-5 sm:p-7 text-slate-900 dark:text-slate-100"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="border-b border-slate-200 dark:border-slate-800 pb-4 mb-5">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-1">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">Registrado en web: {lead.createdAt || 'Reciente'}</span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3">
            <span className="text-xs font-semibold px-2.5 py-1 rounded-lg bg-[#0d233a] dark:bg-slate-800 text-white">
            ...
            <span className="text-xs text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-lg">
        ...
        {activeTab === 'info' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2 text-slate-700 dark:text-slate-300">
                <p className="font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider text-[11px]">
                ...
                {lead.selectedPropertyTitle && (
                  <div className="text-amber-900 dark:text-amber-400 font-medium pt-1">
            ...
            <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-3">
              <p className="font-bold text-slate-800 dark:text-slate-200 text-xs uppercase tracking-wider">
                Actualizar Estado Comercial
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <select
                  ...
                  className="px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-700 text-xs bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100"
                >
              ...
              <button
                ...
                className="bg-[#0d233a] hover:bg-[#163b5c] dark:bg-amber-500 dark:hover:bg-amber-400 dark:text-slate-950 disabled:bg-slate-300 dark:disabled:bg-slate-800 dark:disabled:text-slate-500 text-white font-semibold px-4 py-2 rounded-xl text-xs transition cursor-pointer"
              >
        ...
        {activeTab === 'atribucion' && (
          <div className="space-y-4">
            <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 space-y-4">
              ...
              <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-xl p-3.5 text-xs text-amber-950 dark:text-amber-200 space-y-1">
              ...
              {lead.attributionStatus === 'confirmado' ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                  <div>
                    <span className="text-slate-400 block text-[10px]">Asesor Asignado:</span>
                    <strong className="text-slate-900 dark:text-slate-100">{lead.attributionAdvisor || 'Asesor Asignado'}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[10px]">Folio / Referencia Inmobiliaria:</span>
                    <strong className="text-slate-900 dark:text-slate-100">{lead.attributionReference || 'Sin referencia'}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[10px]">Fecha efectiva de registro:</span>
                    <strong className="text-slate-900 dark:text-slate-100">{lead.attributionConfirmedAt}</strong>
                  </div>
                  <div>
                ...
                <div className="bg-slate-100 dark:bg-slate-800/80 rounded-xl p-3 text-xs text-slate-600 dark:text-slate-300">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/60 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-slate-700 dark:text-slate-300">
                  <div>
                    ...
                    <strong className="text-sm text-slate-900 dark:text-slate-100">{lead.appointmentRequest.preferredDate}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block">Franja horaria solicitada:</span>
                    <strong className="text-sm text-slate-900 dark:text-slate-100">{lead.appointmentRequest.timeSlot}</strong>
                  </div>
                  {lead.appointmentRequest.notes && (
                    <div className="col-span-2 bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <div className="pt-2 border-t border-slate-200 dark:border-slate-700 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                    <input
                      ...
                      className="px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100"
                    />
                    <input
                      type="time"
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    ...
                    <button
                      ...
                      className="bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold px-4 py-2.5 rounded-xl text-xs transition cursor-pointer"
                    >
                      Rechazar / Cancelar
                    </button>
                  </div>
          ...
          <div className="space-y-4">
            <form onSubmit={handleAddNote} className="space-y-2">
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-xs focus:ring-2 focus:ring-[#0d233a] dark:focus:ring-amber-500 focus:outline-none"
              ></textarea>
              <button
                type="submit"
                className="bg-[#0d233a] hover:bg-[#163b5c] dark:bg-amber-500 dark:hover:bg-amber-400 dark:text-slate-950 text-white font-semibold px-4 py-2 rounded-xl text-xs transition cursor-pointer"
              >
                Guardar Nota
              </button>
            </form>
              ...
              {(lead.internalNotes || []).length > 0 ? (
                (lead.internalNotes || []).map((note) => (
                  <div key={note.id} className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs space-y-1">
                    ...
                    <p className="text-slate-800 dark:text-slate-200 leading-relaxed">{note.content}</p>
                  </div>
                ))
              ) : (
                ...
                (lead.auditHistory || []).map((event) => (
                  ...
                  >
                    ...
                    <div className="space-y-0.5 flex-1">
                      ...
                      <p className="font-semibold text-slate-800 dark:text-slate-200">{event.action}</p>
                      {event.reason && (
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4 text-slate-900 dark:text-slate-100">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-xs focus:ring-2 focus:ring-[#0d233a] dark:focus:ring-amber-500 focus:outline-none"
                />
                {revealError && <p className="text-[11px] text-red-600 dark:text-red-400">{revealError}</p>}
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  ...
                  className="bg-amber-600 hover:bg-amber-500 text-white font-bold px-4 py-2 rounded-xl text-xs shadow cursor-pointer"
                >
                  Confirmar y Revelar
                </button>
              </div>
